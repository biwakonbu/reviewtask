...
func TestReleaseScriptDryRunMode(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantExit: 0,
			wantOut:  []string{"DRY RUN: Simulating release preparation", "DRY RUN: Skipping prerequisites check"},
		},
		{
...
func TestVersionEmbedding(t *testing.T) {
	...
	buildCmd := exec.Command("go", "build", "-ldflags", "-X main.version="+version, "-o", "test-binary", ".")
