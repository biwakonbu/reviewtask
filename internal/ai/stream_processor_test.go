...
func TestStreamProcessor_WorkerPool_ConcurrencyControl(t *testing.T) {
	...
	mockProcessor := func(ctx CommentContext) ([]TaskRequest, error) {
		...
		// time.Sleep(10 * time.Millisecond) // Commented out to keep tests fast
	...
	// Process comments through stream processor
	tasks, err := processor.ProcessCommentsStream(comments, mockProcessor)
	...
	// Note: maxActiveWorkers tracking shows the effectiveness of Worker Pool pattern
...
func TestStreamProcessor_WorkerPool_JobDistribution(t *testing.T) {
	...
	mockProcessor := func(ctx CommentContext) ([]TaskRequest, error) {
		mu.Lock()
		processedComments[ctx.Comment.ID] = true
		mu.Unlock()
	...
	if len(processedComments) != numComments {
		t.Errorf("Expected all %d comments to be processed, got %d", numComments, len(processedComments))
	}
	...
	// Verify each comment was processed exactly once
	for i := 1; i <= numComments; i++ {
		if !processedComments[int64(i)] {
			t.Errorf("Comment %d was not processed", i)
		}
	}
}
...
func TestStreamProcessor_WorkerPool_ErrorHandling(t *testing.T) {
	...
	mockProcessor := func(ctx CommentContext) ([]TaskRequest, error) {
		if ctx.Comment.ID == 2 {
			return nil, errors.New("worker error for comment 2")
		}
		return []TaskRequest{
			{
				Description: "Task for comment " + string(rune('0'+ctx.Comment.ID)),
...
func TestStreamProcessor_WorkerPool_NoGoroutineLeaks(t *testing.T) {
	...
	mockProcessor := func(ctx CommentContext) ([]TaskRequest, error) {
		return []TaskRequest{
			{
				Description: "Task " + string(rune('0'+ctx.Comment.ID)),
				Priority:    "medium",
				Status:      "todo",
			},
		}, nil
